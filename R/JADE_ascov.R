ASCOV_JADE <- function(sdf,supp=NULL,A=NULL,...)
{
  ...
  for(j in 1:p){ 
    moment3[j]<-integrate(Vectorize(function(x){sdf[[j]](x)*x^3}),
  ...
  for(i in 1:p){
   for(j in 1:p){
    if(i!=j){ 
      ASVij<-kurt[i]^2*(moment6[i]-moment3[i]^2-kurt[i]^2-6*kurt[i]-9)+
            kurt[j]^2*(moment6[j]-moment3[j]^2+-6*kurt[j]-9)
 
      ASVij<-ASVij/((kurt[i]^2+kurt[j]^2)^2)
        
      ASCOVij<--kurt[i]^2*(moment6[i]-moment3[i]^2-kurt[i]^2-6*kurt[i]-9)-
        kurt[j]^2*(moment6[j]-moment3[j]^2-kurt[j]^2-6*kurt[j]-9-kurt[i]^2)
