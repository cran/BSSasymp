...
ASCOV_SOBI<-function(psi,taus,Beta=NULL,A=NULL)
{
  ...
  for(j in 1:p){
   for(i in 1:p){
    if(i!=j){
      ASV<-.C("ascov", as.double(as.vector(F_tau)),as.double(as.vector(Lambda)),
      ...
      ASCOV<-ASCOV+ASV[2]*kronecker(tcrossprod(diag(p)[,i],diag(p)[,j]),tcrossprod(
             diag(p)[,j],diag(p)[,i]))+ASV[1]*kronecker(tcrossprod(diag(p)[,j],
             diag(p)[,j]),tcrossprod(diag(p)[,i],diag(p)[,i]))
    }  

    if(i==j) ASCOV<-ASCOV+0.25*D_lm(F_tau,0,0,Beta)[i,i]*kronecker(tcrossprod(diag(p)[,i],
